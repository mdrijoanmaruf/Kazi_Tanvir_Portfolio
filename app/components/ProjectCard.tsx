...
export default function ProjectCard({ title, image, link, category }: ProjectCardProps) {
  const getCategoryIcon = (category: string) => {
    if (category === 'Machine Learning' || category === 'AI/ML' || category === 'Data Science') {
      return (
  ...
  const getCategorylinear = (category: string) => {
    if (category === 'Machine Learning' || category === 'AI/ML' || category === 'Data Science') {
  ...
  const hasValidImage = image && image !== '/img/portfolio/default.jpg' && !image.includes('undefined');
    ...
    <div className="group bg-white rounded-2xl border border-gray-200 hover:border-purple-600 overflow-hidden transition-all duration-300 hover:shadow-lg">
      ...
      <div className="relative h-56 w-full overflow-hidden bg-gray-100">
        {hasValidImage ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-linear-to-br from-purple-100 to-indigo-100">
            <div className="text-center">
              ...
              <p className="text-purple-400 text-sm mt-2 font-medium">{category}</p>
            </div>
          </div>
        )}
        ...
        <div className="absolute top-4 right-4">
          <div className={`flex items-center gap-2 bg-linear-to-r ${getCategorylinear(category)} text-white px-3 py-1.5 rounded-full text-xs font-semibold shadow-lg`}>
